<template>
  <div>
    ...
    <div class="scroller">
      ...
      >
        <div class="stock-info" style="display: flex; align-items: center">
          <div
            style="width: 0.88rem; height: 0.88rem; margin-right: 0.2rem"
          ...
          <div style="flex: 1">
            <div class="stock-info__head">
              ...
              <div class="stock-info__trend" @click="openStockModel(currStock)">
                <img
                  v-lazy="getStaticImgUrl('/static/img/trade/blue-stock.svg')"
                />
              </div>
            </div>
        ...
        <div class="info_boxs" v-if="props.type == 'spot'">
          <div class="info_box">
            <div
              class="amount"
              :class="[currStock.offset == 'buy' ? 'up' : 'down']"
            >
              <div>
                {{ currStock.volume || '--'
                }}{{
    ...
    <div v-if="props.type == 'spot'" class="btns">
      <div
        class="btn btn4 ripple-primary"
        @click="emit('cancel', currStock)"
        v-if="['open', 'none'].includes(currStock.status)"
      >
        <div class="btn_icon">
          <img
            v-lazy="getStaticImgUrl('/static/img/trade/cancel.svg')"
            alt="img"
          />
        </div>
    ...
    <div class="btns" v-else>
      <div
        class="btn btn2 ripple-btn"
        @click="emit('update', currStock)"
        v-if="['none', 'lock', 'open'].includes(currStock.status)"
      >
        <div class="btn_icon">
          <img
            v-lazy="getStaticImgUrl('/static/img/trade/update.svg')"
            alt="img"
          />
        </div>
      ...
      <div
        class="btn btn3 ripple-btn"
        @click="emit('sell', currStock)"
        v-if="['open'].includes(currStock.status)"
      >
        <div class="btn_icon">
          <img
            v-lazy="getStaticImgUrl('/static/img/trade/close.svg')"
            alt="img"
          />
        </div>
