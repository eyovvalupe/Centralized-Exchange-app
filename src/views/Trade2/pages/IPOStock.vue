...
<template>
  <div v-if="token" class="page_ipo_stock">
    <Tabs animated :class="from == 'order' ? 'van-tabs--sub-order' : 'van-tabs--sub'" v-model:active="ipoActive"
      :swipeable="false" @change="ipoOnChange" :color="'var(--ex-primary-color)'" shrink
    ...
    <div class="list" :class="from == 'order' ? '!px-[0] h-[13rem] overflow-auto !pb-[0.6rem]' : ''">
      <div class="item" :style="{ backgroundColor: from == 'trade' ? 'var(--ex-bg-white2)' : '' }"
        v-for="(item, i) in ipoStockList" :key="i" @click="ipoOrderDetail(item)">
        <div class="item_box">
          <div class="name_box">
