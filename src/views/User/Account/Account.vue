...
<template>
  <div class="page page_account">
    ...
    <div class="add_btn ripple-btn" @click="goAddAccount">
      <span class="text-white text-[0.36rem] font-normal font-semibold">{{
        $t('account.add_account_btn')
      }}</span>
    </div>
    ...
    >
      <div class="w-full h-[4rem]">
        ...
        >
          <div class="text-[0.36rem] mb-[0.56rem]">
            {{ t('account.no_kyc_title') }}
          </div>
          <div
            class="flex flex-col items-center mb-[0.48rem] px-[0.4rem] text-center"
          >
          ...
          <div class="w-full flex justify-between px-[0.4rem]">
            <div
              ...
              @click="goBack"
            >
              {{ t('user_page.message_box_cancel') }}
            </div>
            <div
              class="w-[3.16rem] h-[0.8rem] flex items-center justify-center rounded-[1.3rem] bg-primary text-black text-[0.32rem]"
              @click="jump('kyc')"
            >
              {{ t('account.no_kyc_confirm') }}
            </div>
          </div>
        </div>
        <div
          v-else-if="userInfo.kycl2 == 2 && !userInfo.googlebind"
          class="w-full flex flex-col items-center"
        >
          <div class="text-[0.36rem] mb-[0.56rem]">
            {{ t('account.no_google_title') }}
          </div>
          <div
            class="flex flex-col items-center mb-[0.48rem] px-[0.4rem] text-center"
          >
            <span class="text-[0.32rem]">{{
              t('account.no_google_con1')
            }}</span>
            <span class="text-[0.32rem]">{{
              t('account.no_google_con2')
            }}</span>
          </div>
          <div class="w-full flex justify-between px-[0.4rem]">
            <div
