...
<template>
  <div class="kyc_1">
    ...
    <div class="steps" v-if="!checkMode">
      <div
        class="bg-color4 w-[0.56rem] h-[0.56rem] rounded-[50%] flex items-center justify-center"
      >
        <div
          class="bg-primary w-[0.32rem] h-[0.32rem] rounded-[50%] flex items-center justify-center"
        >
    ...
    <div class="steps" v-if="!checkMode">
      <div class="text-[0.28rem] text-color mr-[2.52rem]">
        {{ $t('kyc.first_basic_info') }}
      </div>
      <div class="text-[0.28rem] text-color3">
        {{ $t('kyc.first_upload_card') }}
      </div>
    </div>
    ...
    <div
      ...
      :style="{
        backgroundColor: checkMode
          ? 'var(--ex-bg-color2)'
          : 'var(--ex-bg-color6)',
        borderColor: nameError ? 'var(--ex-error-color)' : '',
      }"
    >
      <input
        :disabled="checkMode"
        ...
        class="ipt"
        type="text"
        :placeholder="t('kyc.first_legal_name')"
        maxlength="20"
    ...
    <div
      ...
      :style="{
        backgroundColor: checkMode
          ? 'var(--ex-bg-color2)'
          : 'var(--ex-bg-color6)',
        borderColor: idError ? 'var(--ex-error-color)' : ''
      }"
    >
      <input
        :disabled="checkMode"
        ...
        class="ipt"
        type="text"
        :placeholder="t('kyc.first_card_id')"
        maxlength="20"
        @focus="idError = false"
      />
    </div>
    ...
    <div
      ...
      :style="{
        backgroundColor: checkMode
          ? 'var(--ex-bg-color2)'
          : 'var(--ex-bg-color6)',
        borderColor: birthdayError ? 'var(--ex-error-color)' : ''
      }"
      @click="checkMode ? (showBottom = false) : (showBottom = true); birthdayError = false"
    >
      <div :class="form.birthday ? 'text-color' : 'text-color4'">
        {{ form.birthday || $t('kyc.first_birthday_placeholder') }}
      </div>
      <div class="calendar-icon">
        <img v-lazy="getStaticImgUrl('/static/img/user/calendar.svg')" alt="" />
      </div>
    </div>
    ...
    <!-- 账号验证 -->
    <AccountCheck :from="'kyc'" ref="accountCheckRef" />
  </div>
</template>
...
<script setup>
  ...
  const checkMode = computed(() => {
    return (
      props.kycInfo.status == 'review' || props.kycInfo.status == 'success'
    );
  });
  ...
  const verifyAction = () => {
    if (accountCheckRef.value.check()) {
      if (!form.value.name || !form.value.idnum || !form.value.birthday) {
        if (!form.value.name) nameError.value = true;
        if (!form.value.idError) idError.value = true;
        if (!form.value.birthday) birthdayError.value = true;
        return;
      }
    }
    submit();
