...
<template>
    <div class="page">
        <HeaderTabs  v-model:active="activeTab" :tabs="[$t('inviteFriends.invite'), $t('inviteFriends.reward')]" @change="changeActiveTab(activeTab,true)">
            <template #before>
                <div class="back" @click="back()">
                    <svg xmlns="http://www.w3.org/2000/svg" width="12" height="20" viewBox="0 0 12 20" fill="none">
                    <path d="M10.4 2L2 10.4L10.4 18.8" stroke="#161616" stroke-width="2.1" stroke-linecap="round" stroke-linejoin="round"/>
                    </svg>
                </div>
            </template>
        </HeaderTabs>
                ...
                <div class="scroll-box">
                    <div class="px-[0.32rem] pt-[0.26rem] pb-[0.64rem]">
                        <div class="invite_banner" :class="['invite_banner--'+locale]">
                            <img class="invite_banner_img" src="/static/img/invite/banner2.png" />
                            <div class="invite_banner_titbox">
                                <div class="invite_banner_tit">{{ $t("inviteFriends.banner_title") }}</div>
                                <div class="invite_banner_amount mt-[0.04rem]"><MoneyText text="$" /><MoneyText v-for="(text,i) in moneyTextArr" :key="i" :text="text" /></div>
                                <div class="invite_banner_desc">{{ $t("inviteFriends.banner_desc") }}</div>
                            </div>
                        </div>

                        <div class="invite_title">{{ $t("inviteFriends.qrcode_title") }}</div>
                        <div class="invite_qrcode">
                            <img :src="codeUrl"/>
                        </div>
                        <div class="btn" @click="saveImg">{{ $t("inviteFriends.save_img") }}</div>

                        <div class="invite_title">{{ $t("inviteFriends.invite_code") }}</div>
                        <div class="input-box">
                            <input v-model="inviteCode" disabled />
                            <div class="copy" @click="copy(inviteCode)">{{ $t("inviteFriends.copy") }}</div>
                        </div>
                        ...
                        <div  class="btn" @click="copy(url)">{{ $t("inviteFriends.copy_url") }}</div>
                    </div>
                </div>
            </SwipeItem>
            <SwipeItem>
                    ...
                    <div class="px-[0.32rem] pt-[0.26rem] pb-[0.64rem]">
                        <div class="achievement">
                            <div class="achievement_cont">
                                <div class="achievement_text">{{ $t("inviteFriends.performance_title") }}</div>
                                <div class="achievement_amount">20000</div>
                            </div>
                            <img class="achievement_bg" src="/static/img/invite/achievement.png"/>
                        </div>
...
const url = ref("https://sunex.game/i-dovefc4x-n/")
