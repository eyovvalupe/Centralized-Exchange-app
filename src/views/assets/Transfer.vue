...
<template>
  <div class="page page_trnsfer">
    ...
    <div class="form">
      <div class="form_box mt-[0.4rem] mb-[0.2rem]">
        <div class="flex w-full flex-col">
          <div
            class="min-w-[1.52rem] h-[0.3rem] flex justify-start items-center mb-[0.28rem] ml-[0.08rem]"
          >
            <span class="text-[0.28rem] text-color2 text-center">{{
              $t('transfer.out')
            }}</span>
          </div>
          <div
          ...
          >
            <div class="flex items-center w-full justify-between">
              <div class="account_item flex-shrink-0">
                ...
                <div
                  class="account_item_icon"
                  v-if="!isEmpty(form.fromCurrency)"
                >
        ...
        <div
          class="input_box w-full flex bg-white1 justify-between items-center h-[1.12rem] rounded-[0.32rem] px-[0.28rem] relative border-[0.02rem] transition"
      ...
      <div class="form_box !h-[3.66rem]">
        ...
        >
          <img
            v-lazy="getStaticImgUrl('/static/img/assets/convert.svg')"
            alt="img"
          />
        </div>
          ...
          >
            <div class="flex items-center w-full justify-between">
              <div class="account_item flex-shrink-0">
                ...
                <div class="item_content mr-[0.1rem]">
                  <span class="monty_span">{{
                    !isEmpty(form.toCurrency) ? form.toCurrency.name : ''
                  }}</span>
                </div>
              </div>
      ...
      <div
        class="rate_tip"
        v-if="!isEmpty(form.fromCurrency) && formType == 'swap'"
      >
    ...
    >
      <div
        class="van-popup-custom-title w-full text-center text-[0.32rem] relative top-[-0.05rem]"
      >
        {{ $t('transfer.confirm_con') }}
      </div>
      <Picker
        :swipe-duration="200"
        :show-toolbar="false"
        ...
        <template #option="option">
          <div class="picker-item">
            <span
              ...
              v-if="!columnList.includes(option.key)"
            >
...
<script setup>
  ...
  const balance = computed(() => {
    ...
    if (form.value.from == 'money') {
      ...
      // 转入
      const w = elseWallet.value.find(
        (item) => item.account == form.value.from,
      );
      return w ? w.amount : 0;
    }
  });
  ...
  const openSafePass = () => {
    if (
      ...
      (userInfo.value.role != 'guest' && AccountCheckRef.value.check())
    ) {
