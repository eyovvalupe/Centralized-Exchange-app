...
<template>
  <div class="page page_assets">
    ...
    <Popup v-model:show="handle" position="bottom" :style="{height: '5.46rem', borderTopRightRadius: '0.36rem', borderTopLeftRadius: '0.36rem'}">
        <div class="w-full relative">
            <div class="absolute top-[0.1rem] right-[0.3rem]" @click="() => handle = false">
              <CancelIcon />
            </div>
            <div class="w-full flex justify-center pt-[0.5rem] text-[0.32rem] text-[#121826] leading-[0.44rem] mb-[0.48rem]">
              {{ t('币种操作') }}
            </div>
            <div class="w-full flex items-center flex-col mb-[0.4rem]">
              <div class="usdt_icon mb-[0.16rem]"></div>
              <div class="text-[0.3rem] text-[#061023]">{{ t('USDT') }}</div>
            </div>
            <div class="w-full flex px-[0.32rem] justify-between">
              <div class="w-[1.565rem] h-[1.565rem] rounded-[0.32rem] bg-[#014cfa1a] flex flex-col items-center pt-[0.16rem]">
                <DepositColorIcon class="mb-[0.16rem]" />
                <div class="text-[#014cfa] text-[0.32rem]">{{ t('充值') }}</div>
              </div>
              <div class="w-[1.565rem] h-[1.565rem] rounded-[0.32rem] bg-[#fa75001a] flex flex-col items-center pt-[0.16rem]">
                <WithdrawColorIcon class="mb-[0.16rem]" />
                <div class="text-[#fa7500] text-[0.32rem]">{{ t('提现') }}</div>
              </div>
              <div class="w-[1.565rem] h-[1.565rem] rounded-[0.32rem] bg-[#00af701a] flex flex-col items-center pt-[0.16rem]">
                <TransferColorIcon class="mb-[0.16rem]" />
                <div class="text-[#00af70] text-[0.32rem]">{{ t('提现') }}</div>
              </div>
              <div class="w-[1.565rem] h-[1.565rem] rounded-[0.32rem] bg-[#e8503a1a] flex flex-col items-center pt-[0.16rem]">
                <TransferOutColorIcon class="mb-[0.16rem]" />
                <div class="text-[#e8503a] text-[0.32rem]">{{ t('提现') }}</div>
              </div>
            </div>
        </div>
    </Popup>
