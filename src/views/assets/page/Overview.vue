...
<template>
  <div class="page_assets_overview">
    ...
    <div class="btns">
      ...
      <div class="btn" :class="{ withdraw_btn: !!assets.money }" style="width: 48%" @click="jump('withdraw', true)">
        <div class="icon_box">
          <div class="btn_icon">
          ...
          <span style="margin-left: 0.1rem; font-size: 0.24rem; font-weight: bold; color: #171d2e">{{ $t('可提') }} {{ assets.money }}</span>
        </div>
        <span>{{ $t('提现') }}</span>
      </div>
      <div class="btn" :class="{ recharge_btn: !!hintNum }" style="width: 48%" @click="showAS = true">
        <div class="icon_box">
          ...
          <div v-if="hintNum" style="position: relative; margin-left: 0.1rem; font-size: 0.24rem; font-weight: bold; color: #171d2e">
            <span>{{ $t('记录状态') }}xxx</span>
            <span class="hint">{{ hintNum }}</span>
          </div>
          <div v-else style="margin-left: 0.1rem">{{ $t('充值') }},{{ $t('提现') }},{{ $t('划转') }}</div>
        </div>
        <span>{{ $t('资金记录') }}</span>
      </div>
    </div>
    ...
    <div class="tabs">
      <div ref="tab1" :key="1" class="tab" @click="rightSwitch1 = !rightSwitch1">
        ...
        <div :class="{ open_tab: rightSwitch1 == true }">
          <div>{{ $t('现金账户') }}</div>
          <!-- <div class="tab_info">{{ $t('现金账户币种合计') }}</div> -->
        </div>
        <div class="amount" :class="{ open_amount: rightSwitch1 == true }">{{ assets.money || '0' }}</div>
        ...
        <div class="rights" style="width: 4.8rem" :class="{ open_tab: rightSwitch1 != true }">
          ...
          <div class="right" style="background-color: #ff9500" @click="jump('transfer', false, { from: 'stock', to: 'money' })">
            <div class="right_icon">
      ...
      <div ref="tab2" :key="2" class="tab" @click="rightSwitch2 = !rightSwitch2">
        ...
        <div class="amount" :class="{ open_amount: rightSwitch2 == true }">{{ new Decimal(assets && assets.stock ? assets.stock : 0).add(assets && assets.stock_value ? assets.stock_value : 0) }}</div>
