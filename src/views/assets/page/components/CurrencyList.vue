<template>
  <div v-if="list.length" v-for="(item, i) in list" class="currency_list">
    ...
    <div class="w-full h-[1.04rem] flex items-center rounded-[0.32rem] bg-color3 mb-[0.12rem] justify-between ripple-btn"
      @click="click(item)">
      ...
      <div class="flex gap-[0.08rem] items-center flex-wrap justify-center" style="max-width: 30%;" v-if="item.block">
        <div class="text-[0.32rem] text-color2 mr-[0.08rem]">{{ $t('assets.wallet_blocked_sim') }}</div>
        <div class="mr-[0.24rem] text-[0.32rem] font-semibold">{{ item.block | 8000 }}</div>
      </div>
