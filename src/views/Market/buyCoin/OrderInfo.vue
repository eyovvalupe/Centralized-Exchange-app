    ...
    <template v-if="active == 1">
      ...
      <div class="top">
        ...
        <div class="copy_icon" @click="copy(currItem.order_no)">
          <img v-lazy="getStaticImgUrl('/static/img/common/copy_default.svg')" alt="copy" />
        </div>
      </div>
      ...
      <div v-if="['waitpayment', 'waitconfirm'].includes(currItem.status)" class="bank">
        <div class="bank_title">
          <div class="bank_icon">
            <img v-lazy="getStaticImgUrl('/static/img/bank/card_type_b.svg')" alt="img" />
          </div>
          <div>银行卡</div>
        </div>
        ...
        <div v-if="currItem.bank_status == 'undone' && currItem.offset == 'buy'"
          ...
          <div style="width: 1rem; height: 1rem; margin-bottom: 0.2rem">
            <img v-lazy="getStaticImgUrl('/static/img/chat/wait.svg')" alt="img" />
          </div>
          <div>等待商家提供银行卡</div>
        </div>
