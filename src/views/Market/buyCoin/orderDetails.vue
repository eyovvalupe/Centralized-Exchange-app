<template>
  <div class="orderDetails">
    <Top
      :title="statusEnum[form.status]?.title"
      class="!z-20"
      :back-func="goBack"
    />
    <!-- 一层容器 tab -->
    <div class="tabs-buy !mb-5">
      <div
        class="tab"
        :class="{ active_tab: ['buy', 'sell'].includes(tabsValue) }"
        @click="changeTab('buy')"
      >
      ...
      <div
        class="tab"
        :class="{ active_tab: tabsValue == 'contactTheMerchant' }"
    ...
    <template v-if="tabsValue !== 'contactTheMerchant'">
      <!-- 二层容器 -->
      <div
        class="mb-[0.2rem] w-full rounded-2xl border border-[#d0d8e2] bg-[#f5f7fc] pt-[0.28rem]"
      >
        <template v-if="['waitpayment', 'waitconfirm'].includes(form.status)">
          <div class="flex items-center px-4">
            ...
            <div class="text-base font-semibold text-[#e8503a]">
              {{ countState }}
            </div>
          </div>
          <div
            class="mb-[0.14rem] w-full px-4 text-xs leading-25 text-[#8f92a1]"
          >
        ...
        <div
          v-else
          class="mb-[0.22rem] text-center text-15 leading-25"
        ...
        <div
          class="box-3 relative h-[4.26rem] w-full rounded-2xl bg-white px-4 py-[0.2rem]"
        >
          ...
          <div
            class="mb-[0.2rem] flex pl-[0.62rem] text-xs leading-17 text-[#8f92a1]"
          >
            <span
          ...
          <template v-if="form.bank_status === 'done'">
            <div class="mb-1 flex text-center">
              <div
                class="mr-1 w-[1.96rem] rounded-tl-2xl bg-[#f5f7fc] leading-35"
              >
                <div class="text-sm text-[#666d80]">
                  {{ $t("account.bank") }}
                </div>
              </div>
            ...
            <div class="mb-1 flex text-center">
              ...
              <div class="w-52 bg-[#f5f7fc] leading-35">
                <div
                  class="flex items-center justify-center text-sm text-[#061023]"
                  @click="copy(form.bank_card_number)"
                >
                  {{ form.bank_card_number }}
                  <img
                    class="ml-1 !size-[0.2rem]"
                    :src="getStaticImgUrl('/static/img/trade/copy.png')"
                    alt=""
                  />
                </div>
              </div>
            ...
            <div class="flex text-center">
              <div
                class="mr-1 w-[1.96rem] rounded-bl-2xl bg-[#f5f7fc] leading-35"
              >
                <div class="text-sm text-[#666d80]">
          ...
          <div v-else class="flex flex-col items-center justify-center">
            <img
              class="mb-[0.2rem] !h-18 !w-[1.8rem]"
              :src="getStaticImgUrl('/static/img/market/waitFor.svg')"
              alt=""
            />
            <div class="text-xs text-[#8f92a1]">
      ...
      <div class="h-[2.3rem] w-[6.86rem] rounded-2xl bg-[#f5f7fc]">
        ...
        >
          <div class="bottom_nav_item">
            <div
              class="mb-[0.2rem] w-[1.9rem] text-base font-semibold leading-none text-[#061023]"
            >
              {{ form.totalprice }}
            </div>
      ...
      <!-- 底部button -->
      <div
        v-if="['waitpayment', 'waitconfirm'].includes(form.status)"
        class="van-safe-area-bottom absolute inset-x-4 bottom-0 mb-5 flex justify-between text-16 text-[#666D80]"
      >
        <div
          class="mr-4 flex-1 cursor-pointer rounded-3xl border border-[#d0d8e2] text-center leading-48"
