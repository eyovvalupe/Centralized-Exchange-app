...
<template>
  ...
  >
    ...
    >
      ...
      <polygon
        :points="shadowPoints"
        :style="`fill:url(#${Number(
          props.ratio
        )}); stroke: none; opacity: 1; width:100%`"
      ></polygon>
      <polyline
        :points="props.points"
        :style="`fill: none; stroke: ${color}; stroke-width:${polylineStrokeWidth}; width:100%`"
      ></polyline>
      <defs style="width: 100%" v-if="graphColorGradient">
        <linearGradient
          style="width: 100%"
          :id="props.ratio + ''"
          x1="0%"
...
const graphColorGradient = computed(() => store.state.graphColorGradient);

onMounted(() => {
  setTimeout(() => {
    store.commit("setGraphColorGradient", true)
  }, 3000);
});
const root = ref();
...
const shadowPoints = computed(() => {
  ...
  const shadowPoints =
    ...
    ` ${points[points.length - 1][0]},${svgHeight} ${
      points[0][0]
    },${svgHeight}`;
  return shadowPoints;
});
