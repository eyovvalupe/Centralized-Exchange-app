<template>
  <router-view v-slot="{ Component }">
    <div v-show="pageLoading" class="full_page_loading">
      <!-- 首页骨架屏 -->
      <Skeleton class="max-width home-skeleton" v-if="routeName == 'home'" style="
        ...
        <template #template>
          ...
          <div class="box top">
            ...
            <div class="title">
              <span style="color: #f19009">{{ t('home.landing_con1') }}{{ t('home.landing_con2') }}</span><br />{{ t('home.landing_con3') }}
            </div>
            <div>{{ t('home.landing_con4') }}</div>
          </div>
          <div v-if="!token" class="login-btn">{{ t('trade.stock_opening_token_login') }}/{{ t('trade.stock_opening_token_register') }}</div>
          <div v-else style="height: 1.2rem"></div>

          <div class="box dirs">
            <div class="dir-b" :style="{
          ...
          <div class="box banners-box">
            ...
            <div>
              ...
              <div class="right-banner" style="margin-top: 0.24rem; height: 2.36rem"></div>
            </div>
          </div>
        </template>
      </Skeleton>
      <Loading style="
        ...
        " v-else :type="'circular'" :size="44" color="var(--ex-primary-color)" />
    </div>
...
onMounted(() => {
  ...
  document.body.addEventListener(
    ...
    function (e) {
      // 判断触摸目标是否是按钮  水波纹效果
      const button =
        e.target.closest('.ripple-btn') ||
        e.target.closest('.ripple-primary');
      if (button) {
        // 创建水波纹元素
  ...
  style.innerHTML = `
    ...
    .van-tabs--top .van-sticky  >div>.van-tabs__wrap>.van-tabs__nav>.van-tab--active::after {
      background-image: url(${getStaticImgUrl('/static/img/common/active_tab.svg')});
    }
    ...
    '/static/img/common/search.svg',
  )});
      }

      .page_bank .icon_ok {
        background-image: url(${getStaticImgUrl(
    '/static/img/app/success.svg',
  )});
      }

      .page_withdraw .card_box {
        background-image: url(${getStaticImgUrl(
    '/static/img/assets/withdraw_wallet_bg.svg',
  )});
    ...
    '/static/img/assets/withdraw_wallet_bg.svg',
      ...
      .invite_banner1 {
        background-image: url(${getStaticImgUrl(
    '/static/img/user/achievement.svg',
  )});
        background-size: cover;
      }
    `;
